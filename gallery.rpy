...
define __MAX_PAGE_COUNT = int(__ceil(float(len(replay_defs)) / __ITEM_COUNT))
...
screen gallery_screen():
    ...
    use game_menu(_(u"Gallery")):
        grid __COLS __ROWS:
            ...
            $ active_button_count = __ITEM_COUNT - (__ITEM_COUNT - min(__ITEM_COUNT, len(replay_defs) - list_offset))

            for i in range(active_button_count):
                $ item = replay_defs[list_offset + i]
                imagebutton:
                    idle item[0]
        ...
        textbutton u"Change names":
            action ShowMenu(u"name_change_screen")
            xalign 0.5
            yalign 0.999
            text_size BOTTOM_TEXT_SIZE_

transform grid_scale:
    fit "contain"
    xysize (0.9, 0.9)
